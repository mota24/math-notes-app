...
export function textLines(s: Pick<Stroke, 'text' | 'size' | 'points' | 'family' | 'bold' | 'italic'>): string[] {
  ensureTextFont();
  ensureStyleFont(s);
  const [a, b] = s.points;
  const width = Math.abs((b?.[0] ?? a[0]) - a[0]);
  return wrapText(s.text ?? '', textInnerWidth(width, s.size), textMeasure(s.size, s));
}
